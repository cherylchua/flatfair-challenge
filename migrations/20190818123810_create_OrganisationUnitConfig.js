exports.up = async function(knex) {
    return await _createOrganisationUnitConfigsTable(knex).then(
        await _createOrganisationUnitsTable(knex)
    );
};

exports.down = async function(knex) {
    return await knex.schema
        .dropTableIfExists('organisationunitconfigs')
...
async function _createOrganisationUnitsTable(knex) {
    return await knex.schema.createTableIfNotExists(
        ...
        function(table) {
            ...
            table
                .foreign('config_id')
                .references('organisationunitconfigs.id')
                .onDelete('CASCADE');
            table.timestamps(false, true);
